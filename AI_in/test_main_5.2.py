# deepseek_api 5.2 测试版本
'''
1.优化了方案生成器
2.优化显示设置
3.修了由于多次访问导致的token获取失败
4.修复了由于大模型的切换导致方案生成失败
'''
# 官方库
import sys
import yaml
...
def choice_Large_Model():
    ...
    1.DeepSeek-R1:
        推理模型
        输入价格：2 RMB/M tokens  输出价格：8 RMB/M tokens
    2.DeepSeek-V3:
        非推理模型
        输入价格：1 RMB/M tokens  输出价格：4 RMB/M tokens
    3.DeepSeek-R1-Distill-Liama-70B:
        推理模型
        输入价格：0 RMB/M tokens  输出价格：0 RMB/M tokens
    经同等token输入检测：
     DeepSeek-R1 ≈ DeepSeek-R1-Distill-Liama-70B > DeepSeek-V3
     且非推理模型反应时间是推理模型反应时间的一半
    :return:base_url, api_key, model , Large_model
    '''
